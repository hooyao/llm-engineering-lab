...
- The learner's first D8 payoff run selected a missing `Read` path and aborted
  with `baseline failed: Tool 'Read' failed: File not found`. The file tool and
  `AgentLoop` were already behaving correctly: the exception became a
  model-visible error result so the next model round could recover. The demo
  consumer incorrectly treated every `AgentEvent.Error` as terminal.
- Astra PR #13 added typed `AgentEvent.ToolFailure` for recoverable tool
  exceptions and reserved `AgentEvent.Error` for terminal turn failures. CLI and
  demo consumers now render `ToolFailure` without stopping; a regression proves
  `ToolUse -> ToolFailure -> ToolResult -> TextDelta` and two model calls.
- `MultiAgentDemo` now prints bounded tool arguments and explicitly states that
  relative paths resolve from the Astra repository root. Verification: 113/113
  tests, formatter clean, and zero-warning sample Release build.
- The exact parent-root real command then completed: single agent 26,386 tokens/
  22.3s; multi-agent 152,934 tokens/65.1s = 5.80x tokens and 2.92x slower; both
  workers completed with actual overlap and no isolation-marker leak. This is
  still assistant-side verification; the learner must rerun before D8 is done.
- Astra PR #13 is merged and this parent update pins `main@82c357e`.
