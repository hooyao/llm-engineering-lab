...
On a fresh Codex session, **read `notes/progress.md` first**. Top of that file is a
...
- **Agent side** ("continue agent", "Track D", "work on Astra") → read
  ...
  *building*, the agent-core state lives in the **Astra submodule's own
  `progress.md` and `AGENTS.md`** (`agent/refs/Astra/`) — read those too; this
...
```
AGENTS.md                                  ← this file: directives + hardware spec
notes/
...
agent/                                     ← AGENTIC leg (Track D): build a Codex-style agent core by hand
  ...
  refs/                                    ← submodules: Codex-reviews-Codex (teaching),
                                             Codex-sourcemap (source of truth), Astra (your C# impl)
...
2. **Agentic side** — building a Codex-style agent core **by hand** in
   Astra (the user's C# framework), plus the half Codex omits (RAG, agent
...
In peer mode, behave the way Codex already behaves throughout this repo's
...
All persistent notes, decisions, learning logs, and configuration belong inside this repository. Do **not** write to `~/.Codex/.../memory/`, `MEMORY.md`, or any out-of-repo store. If something is worth remembering across sessions, commit it to a file here (e.g. `notes/`, `decisions/`, topic subdirs). Treat the repo as the single source of truth.
